...
3.  **Reverse Geocoding**: Submit incident coordinate queries to the [Nominatim](https://nominatim.org/release-docs/develop/api/Reverse/) API, keep location type (i.e., school, hospital).
...
p1 <- incidents_cum |> 
  ...
  dark_theme_linedraw() +
    theme(axis.text.x = element_text(angle = 35, vjust = 0.75, hjust=.9, size=12)) +
  ggtitle("Current Casualty Rates")
...
airwars_incidents |> 
  ...
  ggtitle("Sentiment Analysis 2023-24")
...
p1 <- airwars_incidents_coord |> 
  ...
  group_by(cluster) |> 
    reframe(sadness = mean(sadness), 
          ...
          anger = mean(anger), 
          "men killed" = mean(men_killed), 
          "children killed" = mean(children_killed), 
          "women killed" = mean(women_killed),
          school = mean(school), 
          hospital = mean(hospital), 
          "place of workship" = mean(place_of_worship),
          "refugee camp" = mean(refugee_camp), 
          artillery = mean(Artillery)) |> 
  pivot_longer(-cluster) |>
  mutate(name=factor(name, levels=c("children killed","women killed","men killed",
                                    "sadness", "fear", "disgust", "anger",
                                    "school", "hospital", "place of workship",
                                    "refugee camp", "artillery"))) |> 
